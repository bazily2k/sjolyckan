...
def _ensure_booking_constraints():
    try:
        with engine.begin() as conn:
            ...
            conn.exec_driver_sql("ALTER TABLE blocked_dates ADD COLUMN IF NOT EXISTS agent_id integer REFERENCES agents(id)")
            conn.exec_driver_sql("ALTER TABLE blocked_dates ADD COLUMN IF NOT EXISTS guest_name varchar(200)")
            conn.exec_driver_sql("ALTER TABLE blocked_dates ADD COLUMN IF NOT EXISTS guest_email varchar(255)")
            conn.exec_driver_sql("ALTER TABLE blocked_dates ADD COLUMN IF NOT EXISTS guest_phone varchar(50)")
            conn.exec_driver_sql("ALTER TABLE blocked_dates ADD COLUMN IF NOT EXISTS guest_country varchar(10)")
            ...
            conn.exec_driver_sql("ALTER TABLE blocked_dates ADD COLUMN IF NOT EXISTS internal_note text")
            conn.exec_driver_sql("ALTER TABLE blocked_dates ADD COLUMN IF NOT EXISTS hidden boolean NOT NULL DEFAULT false")
