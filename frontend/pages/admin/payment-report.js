            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: 'var(--sand)', borderBottom: '1px solid var(--sand-dark)' }}>
                  {['Ref', 'Gäst', 'Ankomst', 'Avresa', 'Belopp', 'Status', 'Förfaller', 'Handp. betald', 'Slutbet. betald', 'Påminnelse 1', 'Påminnelse 2', 'Påminnelse skickad?'].map(h => (
                ...
                {filtered.map(r => {
                  ...
                  return (
                    <tr key={r.booking_id} style={{ borderBottom: '1px solid var(--sand)', background: overdue ? '#fff5f5' : 'white' }}>
                      ...
                      <td style={{ padding: '10px 14px', whiteSpace: 'nowrap', color: r.deposit_paid_at ? 'var(--ink)' : 'var(--ink-pale)' }}>
                        {r.deposit_paid_at ? fmtDate(r.deposit_paid_at) : '–'}
                      </td>
